...
#     is_have_face = True
#
#     exiv_info = pyexiv2.Image(img.src.path)  # 登记图片路径, only could be work for local image
...
#     #     # TODO: need to change to oss2 method to judge whether the path is existed or not
...
#     # if img.stat.is_face:  # already fetch the info
#     #     print('this image already got the face')
#     #     return
...
#     ret = cap.isOpened()
#     while (ret):
#         ret, req_img = cap.read()  # ndarray类型数据：frame
#         if not ret: break
#
#         ret, img_encode = cv.imencode('.jpg', req_img)
#         print(len(img_encode))
#
#         file_BytesIO = BytesIO(img_encode)
#         print(f'file_BytesIO is {file_BytesIO}')
#         # 构造InMemoryUploadedFile
#         file_name = 'demo.jpg'
#         imuf = InMemoryUploadedFile(file_BytesIO, None, file_name, None, None, None)
#         # 如果时远程图片储存可能需要加以下命令：
#         # imuf.seek(0)
#         print(imuf.read)
#
...
#                 continue
#             # code for oss storage
#             fc_obj = save_face_database(img, face, names, bboxs)  # 保存相关信息到数据库
#             save_face_info(fc_obj.face_info.name, face, fc_obj.name)  # 保存人脸信息到磁盘
#             save_src(fc_obj.src.name, img_path, face.bbox)  # 保存人脸图片到磁盘
#             # code for local storage
#             # fc_obj = save_face_database(img, face, names, bboxs)  # 保存相关信息到数据库
#             # save_face_info(fc_obj.face_info.path, face, fc_obj.name)  # 保存人脸信息到磁盘
#             # save_src(fc_obj.src.path, img_path, face.bbox)  # 保存人脸图片到磁盘
...
#         img.stat.is_face = True
#         img.stat.save()
#         print(f'--------------------{img.id} :faces have been stored to the database---------------------')
#
#     cap.release()
#
#
