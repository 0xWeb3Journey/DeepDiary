...
# 我现在需要将图片上传到mcs 服务器，如下是上传的函数代码，不过这个函数的形参filepath表示的是本地路径图片，我现在需要将网络上的图片上传到mcs 服务器，如何修改这个函数，使得可以将网络上的图片上传到mcs 服务器呢？谢谢！
def upload_file_pay(wallet_info, filepath):  # img is an local path, need to be changed to url
